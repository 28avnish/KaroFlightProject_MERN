...
export const adminLogin = async (req, res, next) => {
 ...
 if (user.status === "suspended") {
  return next(new ErrorResponse("Your account has been suspended. Please contact the Super Admin.", 403));
 }
...
export const updateAdminUser = asyncHandler(async (req, res, next) => {
  ...
  const { name, email, roleType, modules , password , status  } = req.body;
